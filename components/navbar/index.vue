		...
		<view class="wx-head-mod" :style="{height:navHeight+'rpx',backgroundColor:navBackgroundColor}">
			<view class="wx-head-mod-nav" :style="{height:navigationBarHeight+'rpx',top:statusBarHeight+'rpx'}">
				<view class="wx-head-mod-nav-content"
					...
					<!-- 文本区 -->
					<view class="wx-head-mod-nav-content-mian flex-align-center">
						<view class="flex-baseline">
							<view @click="navgate(item)" class="title-item" :class="index==0?'title-active':''"
...
<script>
	...
	export default {
		...
		props: {
			...
			backImageUrl: {
				type: String,
				default: '/static/imgs/back-icon.png'
			},
			// 导航栏整体背景颜色
			navBackgroundColor: {
				type: String,
