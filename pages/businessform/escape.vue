			...
			<view class="form-itemblock" >
				...
				<view class="">
					<view class="numinfo flex">
						<text>数量</text>
						<input type="number" class="num-input" v-model="item.registrationInfo.escapeNum" />
					</view>
	...
	export default {
			...
			if(props.businessId) {
				...
				getRegistrationDetail(props.businessId).then(res => {
					...
					this.formattedInfoList.push({
						...
						noteStatus: false,
						registrationInfo: {
							escapeNum: result.registrationInfo.escapeNum, // 繁殖数量
						}
					})
				})
			}
		...
		// ----------------- S -----------------
				...
				if(this.audit){
					...
					sendEscapeForm(params).then(res => {
						if (res.data.code == 0) {
							uni.showToast({
								title: '保存成功'
							})
