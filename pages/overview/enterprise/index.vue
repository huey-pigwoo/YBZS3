<template>
	<view class="overview_enterprise">
		...
		<view class="statistics">
			在<text class="text">{{totalinfo.countytotal||0}}<text class="text-unit">个区县</text></text>内分布有
			<text class="text">{{totalinfo.companytotal||0}}<text
					class="text-unit">家</text></text>{{maintablist[activityidx].name}}企业,共开展
			<text class="text">{{totalinfo.bistypetotal||0}}<text class="text-unit">类</text></text>
			<text class="text">{{totalinfo.bistimetotal||0}}<text class="text-unit">件</text></text>业务,覆盖率达到
			<text class="text">{{totalinfo.fugailv||0}}<text class="text-unit">%</text></text>。总面积为<text
				...
				class="text">{{totalinfo.totalnum||0}}<text class="text-unit">只</text></text>、<text
				class="text">{{totalinfo.animaltypetotal||0}}<text class="text-unit">种动物</text></text>。
		</view>
		<view class="chart-box">
			...
			<view class="chart">
				<qiun-data-charts v-if="chartData" type="ring" :opts="opts" :chartData="chartData" />
				<view v-else style="height: 300rpx;text-align: center;margin-top: 150rpx;color: #666;">
					暂无数据...
				</view>
				<view v-if="controlidx==2" style="position: absolute;right: 30rpx;bottom: 20rpx;z-index: 9;"
					class="flex-baseline">
	...
	export default {
		data() {
			return {
				...
				titlearr: [{
					...
					name: '企业台账',
					url: '/pages/standingbook/enterprise/enterpriselist'
				}],
				statisticsList: [],
				startTime: '',
				endTime: '',
		...
		methods: {
			...
			controlbisclick(idx) {
				...
				this.chartcontrol1[2].name = idx == 1 ? '动物' : '物种'
				this.init()
			},
